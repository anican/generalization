...
class QueryManager:
    ...
    def _load_pgdl(self, model_id):
        if model_id in self.pgdl_models:
            return self.pgdl_models[model_id]
        model_dir = os.path.join(self.pgdl_models_dir, 'model_{}'.format(model_id))
        print(model_dir)
        ...
        self.pgdl_models[model_id] = model
        return model
    ...
    def _load_custom(self, model_id):
        ...
        model_dir = os.path.join(self.custom_models_dir, 'model_{}'.format(model_id))
        print(model_dir)
        if not os.path.exists(model_dir):
            raise OSError("Directory for custom model {} does not exist!"
                          .format(model_id))
        config_path = os.path.join(model_dir, 'config_{}.json'.format(model_id))
        weights_path = os.path.join(model_dir, 'weights_{}.h5'.format(model_id))
    ...
    def query(self, model_id: str, query_batch_size, is_custom=False):
